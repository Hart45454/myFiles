...
defineTypeNameAndDebug(massTransferGudenau, 0);

addToRunTimeSelectionTable(massTransferModel, massTransferGudenau, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

// Construct from components
massTransferGudenau::massTransferGudenau
(
...
void massTransferGudenau::calcMassContribution()
{
    ...
    {
        Info << "massTransferGudenau using scale = " << typeCG_ << endl;
    }
    else if (particleCloud_.cg() > 1)
    {
        scaleDia_=particleCloud_.cg();
        Info << "massTransferGudenau using scale from liggghts cg = " << scaleDia_ << endl;
    ...
    for(int index = 0;index < particleCloud_.numberOfParticles(); ++index)
    {
            ...
            if(cellI >= 0)
            {
                ...
                scalar h =  0.0000211 * pow((Ufluid/Uref),0.7) ;  //particle-to-fluid mass transfer coefficient, Kpf in the paper, m/s
...
void massTransferGudenau::addMassContribution(volScalarField& Sm) const
{
    Sm += SPartFluid_;
}

void massTransferGudenau::addMassCoefficient(volScalarField& Smi) const
{
    if(implicit_)
    {
...
/*scalar massTransferGudenau::Sherwood(scalar voidfraction, scalar Rep, scalar Sc) const
{
...
void massTransferGudenau::massFlux(label index, scalar h, scalar As, scalar Cfluid, scalar Csfluid, scalar cg3)
...
void massTransferGudenau::giveData()
{
...
void massTransferGudenau::postFlow()
