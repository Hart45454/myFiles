...
#include "error.H"
#include "massTransferNewDiffModel.H"
...
defineTypeNameAndDebug(massTransferNewDiffModel, 0);

addToRunTimeSelectionTable(massTransferModel, massTransferNewDiffModel, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

// Construct from components
massTransferNewDiffModel::massTransferNewDiffModel
(
...
void massTransferNewDiffModel::calcMassContribution()
{
    ...
    {
        Info << "massTransferNewDiffModel using scale = " << typeCG_ << endl;
    }
    else if (particleCloud_.cg() > 1)
    {
        scaleDia_=particleCloud_.cg();
        Info << "massTransferNewDiffModel using scale from liggghts cg = " << scaleDia_ << endl;
    ...
    for(int index = 0;index < particleCloud_.numberOfParticles(); ++index)
    {
            ...
            if(cellI >= 0)
            {
                ...
                scalar D0 = exp(-(12100/Tfluid + 2.568)+(1320/Tfluid - 0.554)*Cfluid);
...
void massTransferNewDiffModel::addMassContribution(volScalarField& Sm) const
{
    Sm += SPartFluid_;
}

void massTransferNewDiffModel::addMassCoefficient(volScalarField& Smi) const
{
    if(implicit_)
    {
...
scalar massTransferNewDiffModel::Sherwood(scalar voidfraction, scalar Rep, scalar Sc) const
{
...
void massTransferNewDiffModel::massFlux(label index, scalar h, scalar As, scalar Cfluid, scalar Csfluid, scalar cg3)
...
void massTransferNewDiffModel::giveData()
{
...
void massTransferNewDiffModel::postFlow()
